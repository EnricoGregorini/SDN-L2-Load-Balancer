...
itg_timeout = 60000    # itg timeout in ms
...
class MyFirstApp(app_manager.RyuApp):
    ...
    def _monitor(self):
        while True:
            if self.start_time is not None and time.time() - self.start_time > 3 and self.iperf_sent is False:  #after 2 seconds, servers start
                t1 = threading.Thread(target=self.run_send_script)
                t2 = threading.Thread(target=self.run_recv_script)
                t1.start()
                self.logger.info("ITG RECEIVERS START!!!!!!!!!!!!!!!!!!!!!!")
                time.sleep(1)
                t2.start()
                self.logger.info("ITG SENDERS START!!!!!!!!!!!!!!!!!!!!!!")
    ...
    # function to start the ITG sender script
    def run_send_script(self):
        if topology_number == 1:
            subprocess.run(['sudo', './Scripts/itg_recv.sh', 'h1', 'h3', 'h5', 'h7'])   # itg recv for topology 1
        elif topology_number == 2:
            subprocess.run(['sudo', './Scripts/itg_recv.sh', 'h1', 'h8', 'h3', 'h4', 'h5'])   # itg recv for topology 2
        elif topology_number == 3:
            subprocess.run(['sudo', './Scripts/itg_recv.sh', 'h1', 'h3', 'h5', 'h7', 'h9', 'h11', 'h13', 'h15'])   # itg recv for topology 3

    # function to start the ITG receiver script
    def run_recv_script(self):
        if topology_number == 1:
            subprocess.run(['sudo', './Scripts/itg_send.sh', '200000', '1500', str(itg_timeout), '4', 'h2', '7', 'h8', '1', 'h4', '5', 'h6', '3'])  # itg send for topology 1
        elif topology_number == 2:
            subprocess.run(['sudo', './Scripts/itg_send.sh', '200000', '1500', str(itg_timeout), '5', 'h9', '1', 'h2', '8', 'h7', '4', 'h6', '3', 'h2', '5'])  # itg send for topology 2
        elif topology_number == 3:
            subprocess.run(['sudo', './Scripts/itg_send.sh', '200000', '1500', str(itg_timeout), '8', 'h2', '15', 'h16', '1', 'h4', '13', 'h14', '3', 'h6', '11', 'h12', '5', 'h10', '9', 'h8', '7'])  # itg send for topology 3
    ...
    def create_avg_bw_util_chart(self):
        if self.time_series:
            ...
            if create_chart:
                ...
                if lb_on:
                    fig.savefig('Plots/itg/MyAvgBwChart_lb_on_'+str(topology_number)+'.png', dpi=300)
                else:
                    fig.savefig('Plots/itg/MyAvgBwChart_lb_off_'+str(topology_number)+'.png', dpi=300)
                plt.close()

                # self.logger.info("AVG BW UTILIZATION CHART SAVED!")
            else:
                if lb_on:
                    avg_df.to_csv('Measurements/itg/avg_utilization_lb_on_'+str(topology_number)+'.csv', index=False, lineterminator='\n')
                else:
                    avg_df.to_csv('Measurements/itg/avg_utilization_lb_off_'+str(topology_number)+'.csv', index=False, lineterminator='\n')
    ...
    def switch_features_handler(self, ev):
        ...
        match = parser.OFPMatch(eth_type=0x86dd)  # match all the packets
        actions = ""
        ...
        # self.logger.info("self.time_series is %s", self.time_series)

        self.counter_switch_configured += 1
        if self.counter_switch_configured == len(self.switches):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if stat.port_no != 1 & stat.port_no != 4294967294 & stat.tx_bytes > 0:    # 4294967294 is the port 'fffffffe' loopback interface of hosts (skip it)
                ...
                if self.isSwitch(u) and self.isSwitch(v):
                    ...
                    if u > v:
                        if self.net.edges[(u, v)]['weight'] > 10**-6:
                    ...
                    if float(self.net.edges[(u, v)]['weight']) > 1:
                        self.logger.info("SOMETHING WRONG, EDGE %s HAS MORE TRAFFIC THAN ITS CAPACITY!", self.net.edges[(u,v)])
