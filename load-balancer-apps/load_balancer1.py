...
create_chart = True  # boolean variable to select if the controller has to create directly the chart here (if it is True) otherwise create files containing data and create the charts with another python script
lb_on = True
...
topology_number = 1  # set the topology used for the simulation
...
class MyFirstApp(app_manager.RyuApp):
    ...
    def _monitor(self):
        while True:
            ...
            # if self.start_time is not None and time.time() - self.start_time > 90:
    ...
    def create_avg_bw_util_chart(self):
        if self.time_series:
            ...
            if create_chart:
                ...
                if lb_on:
                    fig.savefig('Plots/MyAvgBwChart_lb_on_'+str(topology_number)+'.png', dpi=300)
                else:
                    fig.savefig('Plots/MyAvgBwChart_lb_off_'+str(topology_number)+'.png', dpi=300)
                plt.close()

                # self.logger.info("AVG BW UTILIZATION CHART SAVED!")
            else:
                if lb_on:
                    avg_df.to_csv('Measurements/avg_utilization_lb_on_'+str(topology_number)+'.csv', index=False, lineterminator='\n')
                else:
                    avg_df.to_csv('Measurements/avg_utilization_lb_off_'+str(topology_number)+'.csv', index=False, lineterminator='\n')
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        self.sr_rx += 1
        if self.sr_rx % 100 == 0:
            self.logger.info("Edge weights: %s", nx.get_edge_attributes(self.net, 'weight'))
